...
set quantGroups {
    ...
    {Car.FzFL Car.FzFR Car.FzRL Car.FzRR Car.Virtual.Trq_1.x}
...
QuantSubscribe Car.Virtual.Trq_1.x
...
set name "roll_to_right_vw"
set testRun "stand_still_vw"

set maxTorque 20000                        ;# [Nm]
set rampTime 40.0                           ;# [s]
set sampleTime 1                            ;# [ms]
...
proc roll_over_check {t0 minLateralForce timeout sampleTime} {
    while {1} {
        ...
        if {($FzFL <= $minLateralForce) && ($FzRL <= $minLateralForce)} {

            set currentTorque [DVARead Car.Virtual.Trq_1.x]
            DVAWrite Car.Virtual.Trq_1.x 0 -1 Abs

            Log "   *  the vehicle starts to roll over at"
            Log "   *  torque = $currentTorque Nm"

            break
        }

        if {($FzFR <= $minLateralForce) && ($FzRR <= $minLateralForce)} {
...
DVAWrite Car.Virtual.Trq_1.x $maxTorque -1 AbsRamp [expr int($rampTime * 1000)]
set t0 [DVARead Time]
